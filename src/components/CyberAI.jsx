...
const CyberAI = () => {
  ...
  const [messages, setMessages] = useState([
    {
      role: "ai",
      text: "👋 Welcome to Amal Cyber Lab AI. How can I assist your security inquiry today?",
    },
  ]);
  ...
  const handleSend = () => {
    ...
    let response =
      "🔍 Analyzing request... I am trained to discuss Amal's Azure labs and Cyber projects.";

    const query = trimmedInput.toLowerCase();

    if (query.includes("azure") || query.includes("az-500")) {
      response =
        "Amal is highly focused on AZ-500. His labs include Sentinel SIEM, Azure WAF, and PIM implementation.";
    } else if (
      query.includes("medium") ||
      query.includes("article")
    ) {
      response =
        "Amal has published 8+ technical writeups on Medium covering SOC automation and Threat Hunting.";
    } else if (
      query.includes("contact") ||
      query.includes("hire")
    ) {
      response =
        "You can contact Amal via the contact form below or through his LinkedIn profile.";
    }
    ...
    <div className="fixed bottom-28 right-8 z-[9999]">
      {isOpen ? (
        <div className="w-[320px] bg-slate-950/95 backdrop-blur-2xl border border-emerald-500/30 rounded-2xl shadow-[0_20px_50px_rgba(16,185,129,0.2)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
          ...
          <div className="p-4 bg-emerald-500/10 border-b border-white/5 flex justify-between items-center text-emerald-400 font-bold text-[10px] uppercase tracking-[0.2em]">
            ...
            <button
              ...
              aria-label="Close Cyber AI Assistant"
          ...
          <div className="flex border-t border-white/5 p-3 bg-black/20">
            <input
              ...
              aria-label="Ask Cyber AI Assistant"
            />
            ...
            <button
              ...
              className="p-2 text-emerald-400 hover:scale-110 active:scale-90 transition-all"
            >
              <Send size={16} />
            </button>
          </div>
        ...
        <button
          ...
          aria-label="Open Cyber AI Assistant"
