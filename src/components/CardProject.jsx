...
const CardProject = ({ Img, Title, Description, Link: ProjectLink, id }) => {
  ...
  const getTags = (title = "") => {
    ...
    if (t.includes("soar"))
      return ["SOAR", "Logic Apps", "Automation"];

    if (t.includes("ddos"))
      return ["Azure", "DDoS", "Network"];

    if (t.includes("honeypot"))
      return ["Threat Intel", "SOC", "Attack Map"];
    ...
    if (t.includes("waf"))
      return ["WAF", "App Gateway", "Web Security"];
  ...
  const getDifficulty = (title = "") => {
    ...
    if (t.includes("nessus") || t.includes("waf"))
      return "Intermediate";
  ...
  return (
    <div className="group w-full h-full flex items-stretch [perspective:1200px]">
      ...
      >
        ...
        {/* Main Card */}
        <div className="relative h-full min-h-[440px] flex flex-col justify-between rounded-3xl bg-[#08111d]/90 backdrop-blur-2xl border border-white/10 p-5 pb-6 overflow-hidden shadow-xl transition-all duration-500 group-hover:border-emerald-500/40 group-hover:shadow-emerald-500/10">
          ...
          {/* IMAGE SECTION */}
          <div className="relative rounded-2xl overflow-hidden bg-[#050812] aspect-[16/10] flex items-center justify-center flex-shrink-0 border border-white/5">

            <img
              src={Img}
              alt={Title}
              loading="lazy"
              className="w-full h-full object-contain object-center rounded-xl transition duration-700 group-hover:scale-[1.03]"
              onError={(e) => {
                e.currentTarget.src =
